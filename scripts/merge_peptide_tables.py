...
def main():
    ...
    # lookup approach for peptide -> proteins
    ...
    # map to HGNC ID
    (
        pd.DataFrame.from_dict(peptide_assignments, orient='index')
        .map(lambda l: reduce(
            set.union,
            (
                accession2hgncids.get(acc, set())
                for acc in (
                    rowid2accession[rowid]
                    for rowid in l
                )
            )
        ))
        .map(lambda l: sorted(l, key=natural_sort_key))
        .map(lambda l: ";".join(l))
        .set_axis(["hgnc_ids", "non_redundant_hgnc_ids", "semi_non_redundant_hgnc_ids"], axis=1)
        .rename_axis("peptide", axis=0)
        .reset_index()
    ).to_csv(args.outfile, sep="\t", header=True, index=False)
